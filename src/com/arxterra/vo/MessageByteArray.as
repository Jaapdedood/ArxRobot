package com.arxterra.vo
{
	...
	/**
	 * Note: Implements IExternalizable, IPilotMessageSerialize.
	 */
	public class MessageByteArray implements IExternalizable, IPilotMessageSerialize
	{
		...
		public function toSFSObject ( ) : ISFSObject
		{
			...
			sfso.putByteArray ( 'b', _messageBytesClone() );
			return sfso;
		}
		...
		public function clearValueBytes ( ) : void
		{
			if ( _messageBytes.length > 1 )
			{
				_messageString = '';
				_messageBytes.length = 1;
			}
		}
		
		/**
		 * @param byteArray Source bytes
		 * @param byteCount Number of bytes that need to be written
		 * @param append True if accumulating multiple values, as with the telemetry streaming flow protocol
		 * @return <b>true</b> if byteCount bytes were available to write, <b>false</b> otherwise
		 */
		public function writeValueBytes ( byteArray:ByteArray, byteCount:uint, append:Boolean = false ) : Boolean
		{
			if ( byteArray.bytesAvailable < byteCount )
				return false; // return
			
			var uLen:uint = _messageBytes.length;
			if ( uLen > 0 )
			{
				_messageString = '';
				byteArray.readBytes ( _messageBytes, ( append ? uLen : 1 ), byteCount );
			}
