package com.arxterra.vo
{
	 ...
	 * superclass McuMessage thus includes 8 bytes for each.</p>
	 * <p>Use <b>messageBytesByCoordsMode</b>
	 * to get the ByteArray format specified, which will be one of the following:</p>
	 * <ul><li><b>64-bit double precision floats</b>, exactly as they are stored</li>
	 * <li><b>32-bit single precision floats</b>, which are standard for most Arduinos
	 * and similar MCUs</li>
	...
	public class CoordinatesMessage extends McuMessage
	{
		// CONSTANTS
		 ...
		 * configured Coordinates data type expected by the MCU.
		 */
		override public function get messageBytesForMcu ( ) : ByteArray
		{
			if ( __uCoordsMode == COORDS_MODE_INTEGER )
				return _messageBytesIntegerDmDegrees ( ); // NOTE: may be calling subclass override
			
			if ( __uCoordsMode == COORDS_MODE_FLOAT )
				return _messageBytesSinglePrecision ( ); // NOTE: may be calling subclass override
		 ...
		 * messageBytesForCoordsMode sent to Mcu.
